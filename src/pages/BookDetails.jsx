...
const BookDetails = () => {
  ...
  // find the book by its slug or _id (make sure id in URL matches your slug)
  const book = books.find((b) => b.slug === id || b._id === id)
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-[#FFFAEB] pt-28 pb-24">
        ...
        <svg
          className="absolute w-10 top-24 right-20 animate-float-medium opacity-30 z-0"
          viewBox="0 0 24 24"
          fill="#86efac"
        >
          <circle cx="12" cy="8" r="6" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-12 items-start animate-fadeIn">
          ...
          <div className="text-gray-800 max-w-xl font-story">
            <h2 className="text-4xl md:text-5xl font-bold mb-4 font-fun text-ink">
              {book.name}
            </h2>
            <p className="text-2xl text-amber-700 font-bold mb-4">
              {currency}
              {book.price}
            </p>
        ...
        {book.previewPages?.length > 0 && (
          <div className="relative z-10 max-w-7xl mx-auto px-4 mt-16">
            <h3 className="text-2xl font-fun mb-6 text-ink">Sneak Peek Inside</h3>
            <div className="flex flex-wrap gap-6 justify-center md:justify-start">
              {book.previewPages.map((pageSrc, i) => (
                <img
                  key={i}
                  src={pageSrc}
                  alt={`${book.name} page ${i + 1}`}
                  className="w-[160px] h-[220px] object-cover rounded-xl shadow-md hover:scale-105 transition-transform duration-300"
                />
              ))}
            </div>
          </div>
